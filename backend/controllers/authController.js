...
const { getUserCacheFilePath, writeCacheFile,initCacheFromDefaultIfNeeded } = require('../services/cacheService');

exports.saveUser = async (req, res) => {
  const { uid, email, role, name,lastName,streamId,actualQuiz } = req.body;
  let uData={
      email,
      name,
      lastName,
  ...
  try {
    await db.collection("users").doc(uid).set(uData);
...
initCacheFromDefaultIfNeeded(userId);
  const cachePath = getUserCacheFilePath(userId);

  // 1. Vérifie s’il existe un cache
  if (fs.existsSync(cachePath)) {
    const cachedData = JSON.parse(fs.readFileSync(cachePath, 'utf8'));
    if(cachedData.name===""){
    const userDoc = await db.collection('users').doc(userId).get();
    cachedData.email=email;
    cachedData.name=userDoc.data().name;
    cachedData.role=userDoc.data().role;
    cachedData.streamId=userDoc.data().streamId;
  }
  res.json(cachedData);
    // 2. Rafraîchit en tâche de fond
    refreshUserDataCache(userId, cachePath);
    return;
  }

  // 3. Sinon, récupère depuis Firebase et crée le cache
  try {
    const dashboardData = await generateUserDashboardData(userId);
    writeCacheFile(cachePath, dashboardData);
...
// Fonction qui régénère le cache silencieusement
async function refreshUserDataCache(userId, cachePath) {
  try {
    
    const dashboardData = await generateUserDashboardData(userId);
    writeCacheFile(cachePath, dashboardData);
  } catch (error) {
    console.warn(`Failed to refresh user cache for ${userId}`, error.message);
  }
}

async function generateUserDashboardData(userId) {
 ...
 /* if (resultsSnap.empty) {
  ...
  const averageScore = Math.round(totalScore / pastQuizzes.length);
  ...
  return {
    ...
    nextQuiz: null,
    stats: {
      completedQuizzes: pastQuizzes.length,
      averageScore,
      skillsMastered: 0,
      attitudesMastered: 0,
      knowledgeMastered: 0,
