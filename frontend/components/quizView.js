...
function renderSummary() {
  ...
  const summaryHTML = quiz.questions.map((q, i) => {
    const userCorrect = successQuestionIds.includes(quiz.questions[i].id);
...
function getStructuredQuizById(quizId) {
  let quiz = null;
  ...
  for (const levelItem of window.all) {
    for (const streamItem of levelItem.streams || []) {
      for (const moduleItem of streamItem.modules || []) {
        for (const quizItem of moduleItem.quizzes || []) {
          if (quizItem.id === quizId) {
            quiz = quizItem;
            parentModule = moduleItem;
            parentStream = streamItem;
            level = levelItem;
            break;
          }
        }
  ...
  if (!quiz || !parentModule) throw new Error('Quiz or module not found');

  // 🔍 Trouver le cours associé à ce module via moduleId
  course = (window.courses || []).find(c => c.moduleId === parentModule.id) || {};

  const knowledge = parentModule.knowledge || [];
  const skills = parentModule.skills || [];
  const attitudes = parentModule.attitudes || [];
...
export async function startQuiz(quizId) {
 ...
 const quizData=getStructuredQuizById(quizId)
setTimeout(()=>{
quizData.id=quizId;
  quiz = quizData;
  currentQuestion = 0;
